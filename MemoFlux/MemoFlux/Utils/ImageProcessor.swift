...
class ImageProcessor {
  ...
  struct CompressionConfig {
    ...
    /// 百炼优化配置：最大尺寸512x512，压缩质量0.4
    static let bailianOptimized = CompressionConfig(
      maxWidth: 512,
      maxHeight: 512,
      compressionQuality: 0.4
    )

    /// 百炼兜底配置：最大尺寸384x384，压缩质量0.3
    static let bailianFallback = CompressionConfig(
      maxWidth: 384,
      maxHeight: 384,
    ...
    // 兼容旧命名，避免其他调用点编译失败。
    static let arkOptimized = bailianOptimized
    static let arkFallback = bailianFallback
  }
